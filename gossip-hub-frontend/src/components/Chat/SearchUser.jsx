...
import { IoMdSearch, IoMdPersonAdd, IoMdClose, IoMdPulse } from "react-icons/io";
...
const SearchUser = () => {
    ...
    return (
        <div className="relative w-full font-inter z-[1000] perspective-1000">
            {/* 🔍 The Restored 3D Engagement Effect */}
            <div className="relative group/search">
                ...
                {/* 🌌 Holographic Floor Flare (Restored) */}
            ...
            {/* 🛰️ The Results Panel (Solid Opaque fix + High Blur) */}
            {results.length > 0 && (
                    ...
                    <div className="p-2 flex flex-col gap-1 overflow-y-auto max-h-[340px] custom-scrollbar bg-[#0B0E14]">
                        {results.map((user) => (
                            <div key={user._id} className="group/item flex items-center justify-between p-4 rounded-xl transition-all hover:bg-white/[0.05] border border-transparent hover:border-white/5">
                                ...
                                <button onClick={() => handleAddFriend(user._id, user.username)} className="bg-white text-black text-[9px] font-black uppercase px-4 py-2 rounded-lg hover:bg-syncra-pink hover:text-white transition-all active:scale-95 shadow-md">
                                    CONNECT
                                </button>
                            </div>
                        ))}
